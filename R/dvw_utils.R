...
dv_create_substitution <- function(dvw, team = NULL, ridx = NULL, in_player = NULL, out_player = NULL, new_setter = NULL) {
    ...
    changedRows <- rotations(dvw, team = teamSelect, start_point_id = current_point_id+1,
    ...
    if(nrow(changedRows$new_rotation)>0) new_xx <- dplyr::left_join(dplyr::select(toreplace, 'point_id'), changedRows$new_rotation, by = "point_id") else new_xx = NULL

    ## The first row of toreplace, which is the rotation line, is kept
    #new_xx[1,]<- toreplace[1,]
    if(!is.null(new_xx)) dvw$plays[dvw$plays$point_id %in% changedRows$new_rotation$point_id, colnames(changedRows$new_rotation)] <- new_xx
